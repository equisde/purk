...
public:
  ...
  virtual void getRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint16_t outsCount,
    std::vector<currency::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result, const Callback& callback) override {
  }
...
CryptoNote::INode* NodeFactory::createNode(const std::string& daemonAddress, uint16_t daemonPort, Logging::ILogger& logger) {
    ...
    CryptoNote::NodeRpcProxy* miNodo = new CryptoNote::NodeRpcProxy(daemonAddress, daemonPort, logger);


    std::unique_ptr<CryptoNote::NodeRpcProxy> node2(miNodo);
