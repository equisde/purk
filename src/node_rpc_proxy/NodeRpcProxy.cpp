...
#include "currency_core/TransactionApi.h"

#include "common/StringTools.h"
#include "currency_core/currency_basic_impl.h"
#include "currency_core/CryptoNoteTools.h"
...
void NodeRpcProxy::resetInternalState() {
  ...
  lastLocalBlockHeaderInfo.hash = currency::null_hash2;
  lastLocalBlockHeaderInfo.prevHash = currency::null_hash2;
...
void NodeRpcProxy::updateBlockchainStatus() {
  currency::COMMAND_RPC_GET_LAST_BLOCK_HEADER2::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_GET_LAST_BLOCK_HEADER2::response rsp = AUTO_VAL_INIT(rsp);
  ...
  if (!ec) {
    ...
    if (blockHash.data != lastLocalBlockHeaderInfo.hash.data) {
  ...
  currency::COMMAND_RPC_GET_INFO2::request getInfoReq = AUTO_VAL_INIT(getInfoReq);
  currency::COMMAND_RPC_GET_INFO2::response getInfoResp = AUTO_VAL_INIT(getInfoResp);
...
std::error_code NodeRpcProxy::doGetBlockHashesByTimestamps(uint64_t timestampBegin, size_t secondsCount, std::vector<Crypto::Hash>& blockHashes) {
  currency::COMMAND_RPC_GET_BLOCKS_HASHES_BY_TIMESTAMPS::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_GET_BLOCKS_HASHES_BY_TIMESTAMPS::response rsp = AUTO_VAL_INIT(rsp);
...
void NodeRpcProxy::getRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint16_t outsCount,
                                          std::vector<currency::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS2::outs_for_amount>& outs,
...
std::error_code NodeRpcProxy::doRelayTransaction(const CryptoNote::Transaction& transaction) {
  currency::COMMAND_RPC_SEND_RAW_TX2::request req;
  currency::COMMAND_RPC_SEND_RAW_TX2::response rsp;
  req.tx_as_hex = toHex(CryptoNote::toBinaryArray(transaction));
...
std::error_code NodeRpcProxy::doGetRandomOutsByAmounts(std::vector<uint64_t>& amounts, uint16_t outsCount,
                                                       std::vector<currency::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS2::outs_for_amount>& outs) {
  currency::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS2::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS2::response rsp = AUTO_VAL_INIT(rsp);
...
static inline void serialize(currency::COMMAND_RPC_GET_BLOCKS_FAST::response& response, ISerializer &s) {
...
std::error_code NodeRpcProxy::doGetNewBlocks(std::vector<Crypto::Hash>& knownBlockIds,
                                             std::vector<CryptoNote::RawBlock>& newBlocks,
                                             uint32_t& startHeight) {
  currency::COMMAND_RPC_GET_BLOCKS_FAST2::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_GET_BLOCKS_FAST2::response rsp = AUTO_VAL_INIT(rsp);
...
std::error_code NodeRpcProxy::doGetTransactionOutsGlobalIndices(const Crypto::Hash& transactionHash,
  ...
  currency::COMMAND_RPC_GET_TX_GLOBAL_OUTPUTS_INDEXES2::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_GET_TX_GLOBAL_OUTPUTS_INDEXES2::response rsp = AUTO_VAL_INIT(rsp);
...
std::error_code NodeRpcProxy::doQueryBlocksLite(const std::vector<Crypto::Hash>& knownBlockIds, uint64_t timestamp,
  ...
  currency::COMMAND_RPC_QUERY_BLOCKS_LITE::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_QUERY_BLOCKS_LITE::response rsp = AUTO_VAL_INIT(rsp);
...
std::error_code NodeRpcProxy::doGetPoolSymmetricDifference(std::vector<Crypto::Hash>&& knownPoolTxIds, Crypto::Hash knownBlockId, bool& isBcActual,
  ...
  currency::COMMAND_RPC_GET_POOL_CHANGES_LITE::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_GET_POOL_CHANGES_LITE::response rsp = AUTO_VAL_INIT(rsp);
...
std::error_code NodeRpcProxy::doGetBlocks(const std::vector<Crypto::Hash>& blockHashes, std::vector<BlockDetails>& blocks) {
  currency::COMMAND_RPC_GET_BLOCKS_DETAILS_BY_HASHES::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_GET_BLOCKS_DETAILS_BY_HASHES::response resp = AUTO_VAL_INIT(resp);
...
std::error_code NodeRpcProxy::doGetTransactionHashesByPaymentId(const Crypto::Hash& paymentId, std::vector<Crypto::Hash>& transactionHashes) {
  currency::COMMAND_RPC_GET_TRANSACTION_HASHES_BY_PAYMENT_ID::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_GET_TRANSACTION_HASHES_BY_PAYMENT_ID::response resp = AUTO_VAL_INIT(resp);
...
std::error_code NodeRpcProxy::doGetTransactions(const std::vector<Crypto::Hash>& transactionHashes, std::vector<TransactionDetails>& transactions) {
  currency::COMMAND_RPC_GET_TRANSACTION_DETAILS_BY_HASHES::request req = AUTO_VAL_INIT(req);
  currency::COMMAND_RPC_GET_TRANSACTION_DETAILS_BY_HASHES::response resp = AUTO_VAL_INIT(resp);
