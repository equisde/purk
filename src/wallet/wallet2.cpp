...
namespace tools
{
std::vector<wallet2::transfer_container::iterator>wallet2::DEFAULT_SELECTED_TRANSFER = {};
//----------------------------------------------------------------------------------------------------
...
void wallet2::process_new_transaction(const currency::transaction& tx, uint64_t height, const currency::block& b)
{
    ...
    if(!outs.empty() && tx_money_got_in_outs)
    {
        ...
        BOOST_FOREACH(size_t o, outs)
        {
            ...
            currency::generate_key_image_helper(m_account.get_keys(), tx_pub_key, o, in_ephemeral, keyImage);
            CHECK_AND_THROW_WALLET_EX(in_ephemeral.pub != boost::get<currency::txout_to_key>(tx.vout[o].target).key,
                                      error::wallet_internal_error, "key_image generated ephemeral public key not matched with output_key");
            
            const auto it = m_key_images.find(keyImage);
            bool skipTransaction = it != m_key_images.end();
            if (skipTransaction)
            {
...
void wallet2::process_new_blockchain_entry(const currency::block& b, currency::block_complete_entry& bche, crypto::hash& bl_id, uint64_t height)
{
    ...
    if(b.timestamp + 60*60*24 > m_account.get_createtime() ||  this->flag_restore_from_zero == true) {
...
/**
 * @brief wallet2::get_payments_by_id
 * @param str_payment_id
 * @param payments
 * @param min_height
 */
void wallet2::get_payments_by_id(const crypto::hash& payment_id, std::list<wallet2::payment_details> &payments, uint64_t min_height) const{
    for (const auto& td : this->m_transfers) {
        transaction tx = td.m_tx;
        crypto::hash curr_payment_id;
        get_payment_id_from_tx_extra(tx, curr_payment_id);
        ...
        if (curr_payment_id == payment_id) {
            ...
            bool r = parse_and_validate_tx_extra(tx, tx_pub_key);
            if (r) {
                if (lookup_acc_outs(m_account.get_keys(), tx, tx_pub_key, outs, tx_money_got_in_outs)) {
//                    std::cout << " - get_payments_by_id  : " << curr_payment_id << std::endl;
//                    std::cout << "      - hash: " << currency::get_transaction_hash(tx) << std::endl;
...
//                        i++;
//                    }
//                    std::cout << "      - tx_money_spent_in_ins: " << print_money(tx_money_spent_in_ins) << std::endl;
//                    std::cout << "      - tx_money_got_in_outs : " << print_money(tx_money_got_in_outs) << std::endl;
//                    std::cout << "      -               amount : " << print_money(td.amount()) << std::endl;
//                    uint64_t received = (tx_money_spent_in_ins < tx_money_got_in_outs) ? tx_money_got_in_outs - tx_money_spent_in_ins : 0;
...
//                    {
                        payment_details payment;
                        payment.m_tx_hash      = currency::get_transaction_hash(tx);
                        payment.m_amount       = /*received*/ td.amount();
                        payment.m_block_height = td.m_block_height;
                        payment.m_unlock_time  = tx.unlock_time;
//                        m_payments.emplace(payment_id, payment);
...
currency::account_public_address wallet2::get_account_public_address() const
{
    return m_account_public_address;
}
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------
/**
 * Helper method which allows to check if particular transaction is unlocked..
 * @param transaction_detail
 * @return
 */
bool wallet2::is_transaction_available(const transfer_details& transaction_detail) {
    if(!transaction_detail.m_spent && is_transfer_unlocked(transaction_detail)
            && currency::is_mixattr_applicable_for_fake_outs_counter(boost::get<currency::txout_to_key>(transaction_detail.m_tx.vout[transaction_detail.m_internal_output_index].target).mix_attr, 0)) {
        return true;
    }
    return false;
}
...
/**
 *
 * @param ret_selected_transfers
 * @param total_amount
 * @param total_size_bytes
 */
void wallet2::consolidate_inputs(std::list<std::vector<transfer_container::iterator>> &ret_selected_transfers,
                                 uint64_t* total_amount,
                                 uint64_t* total_size_bytes)
{
    transfer_container::iterator it = this->m_transfers.begin();
    std::vector<transfer_container::iterator>temp_transfer;

    uint64_t size{0};

    while (it != this->m_transfers.end()) {

        uint64_t curr_size = get_object_blobsize((*it).m_tx);
        uint64_t amount = (*it).amount();

        // Check if amount of current transaction lass than one PURK
        if (amount < ANTI_OVERFLOW_AMOUNT && is_transaction_available((*it))) {
            *total_size_bytes += curr_size;
            *total_amount += amount;

            if ((size + curr_size) < CURRENCY_MAX_TRANSACTION_BLOB_SIZE) {
                size += curr_size;
                temp_transfer.push_back(it);
            } else {
                if (temp_transfer.size() > 0) {
                    ret_selected_transfers.push_back(temp_transfer);
                    temp_transfer.clear();
                }
                size = 0;
            }
        }
        if ((++it) == this->m_transfers.end()) {
            if (temp_transfer.size() > 0) {
                ret_selected_transfers.push_back(temp_transfer);
                temp_transfer.clear();
            }
...
/**
 * @brief wallet2::select_transfers
 * @param needed_money
 * @param fake_outputs_count
 * @param dust
 * @param selected_transfers
 * @return
 */
uint64_t wallet2::select_transfers(uint64_t needed_money,
...
void wallet2::transfer(const std::vector<currency::tx_destination_entry>& dsts,
                       ...
                       currency::transaction& _transaction,
                       const std::vector<transfer_container::iterator>& transfers)
{
    transfer(dsts, fake_outputs_count, unlock_time, fee, extra, detail::digit_split_strategy, tx_dust_policy(fee), _transaction, transfers);
    //    std::cout << " - wallet2::transfer: tx size: -> " << currency::get_object_blobsize(_transaction) << std::endl;
    //    uint64_t amount{0};
    //    for (auto item : _transaction.vout) amount += item.amount;
    //    std::cout << "      - transaction vout amount: " << amount << std::endl;
    //    std::cout << "      - fee: " << fee << std::endl;
    //    std::cout << "      - fake_outputs_count: " << fake_outputs_count << std::endl;
}
//----------------------------------------------------------------------------------------------------
/**
 * @brief wallet2::transfer
...
{
    currency::transaction _transaction;
    transfer(dsts, fake_outputs_count, unlock_time, fee, extra, _transaction, transfers);
    //    std::cout << " - wallet2::transfer: tx size: -> " << currency::get_object_blobsize(_transaction) << std::endl;
    //    uint64_t amount{0};
    //    for (auto item : _transaction.vout) amount += item.amount;
    //    std::cout << "      - transaction vout amount: " << amount << std::endl;
...
{
    //    std::cout << "looking for tid: " << txid << std::endl;
    //    for (auto td : m_transfers) {
    //        transaction tx = td.m_tx;
    //        crypto::hash th = get_transaction_hash(tx);
    //        if (txid == th) {
    //            std::cout << " transaction hash = " << th << std::endl;
    //            std::cout << "public key from extra: " << get_tx_pub_key_from_extra(tx) << std::endl;
    //
    //        }
    //    }
