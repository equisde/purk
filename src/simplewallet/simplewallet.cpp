    ...
    , m_refresh_progress_reporter(*this)
{
    ...
    m_cmd_binder.set_handler("sum_inputs", boost::bind(&simple_wallet::suminput, this, _1), "Calculate all the small inputs that have come in to the wallet and their total in KB.");
    std::string max_blob_size = "Combine small input transactions into " + std::to_string(CURRENCY_MAX_TRANSACTION_BLOB_SIZE / 1024) + "KB blocks and send these blocks back to the user";
    m_cmd_binder.set_handler("consolidate_inputs", boost::bind(&simple_wallet::consolidateinputs, this, _1), max_blob_size);
}
//----------------------------------------------------------------------------------------------------
...
/**
 * @brief simple_wallet::suminput will make a calculation on the logged in wallet. It will calculate all the
 * small inputs that have come in to the wallet and their total in KB
 * @param args command arguments.
 * @return
 */
bool simple_wallet::suminput(const std::vector<string> &args) {
    ...
    message_writer(epee::log_space::console_color_green) << "Calculate total of small input transactions:";
    message_writer(epee::log_space::console_color_magenta, false) << "\tblocks: " << count;
    message_writer(epee::log_space::console_color_magenta, false) << "\t  size: " << (total_size / 1024.0f) << " KB";
    message_writer(epee::log_space::console_color_magenta, false) << "\tamount: " << print_money(total_amount) << " PURK";
    return true;
}

/**
 * @brief simple_wallet::consolidateinputs
 * @param args
 * @return
 */
bool simple_wallet::consolidateinputs(const std::vector<string> &args) {
    ...
    message_writer(epee::log_space::console_color_green, true) << "Consolidate blocks:";
    message_writer(epee::log_space::console_color_magenta, false) << "\t    blocks: " << temp_selected_transfers.size();
    message_writer(epee::log_space::console_color_magenta, false) << "\ttotal size: "
                                                                  << total_size_bytes << "b ("
    ...
    if (user_input == "yes") {
        message_writer(epee::log_space::console_color_magenta, false) << "begin processing...";
        ...
        for (auto selected_transfers : temp_selected_transfers) {
            ...
            try {
                message_writer(epee::log_space::console_color_blue, true) << "Send transaction: block[" << block_count << "]";
                message_writer(epee::log_space::console_color_cyan, true) << "\t     Amount: " << print_money(dsts[0].amount) << " PURK";
                message_writer(epee::log_space::console_color_cyan, true) << "\t        Fee: " << print_money(fee) << " PURK";

                m_wallet->transfer(dsts, 0, 0, fee, {}, transaction, selected_transfers);

                message_writer(epee::log_space::console_color_green, true) << "Transaction successfully sent...";

            } catch (const std::exception& e) {
                message_writer(epee::log_space::console_color_red, true) << "Transfer error: reason -> " << e.what();
                LOG_ERROR("Transfer error: " << e.what());
                block_count++;
                continue;
            } catch (...) {
                message_writer(epee::log_space::console_color_red, true) << "Transfer error: reason -> unknown" ;
...
//            m_wallet->get_payments(payment_id, payments);
            m_wallet->get_payments_by_id(payment_id, payments);
